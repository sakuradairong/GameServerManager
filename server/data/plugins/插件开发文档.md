...
- 避免执行不安全的操作
- 遵循最小权限原则
- 已启用插件的 HTML、CSS、JavaScript、图片、字体和媒体文件会作为界面静态资源直接提供，请勿在这些文件中嵌入 token、密码或私钥
- JSON 与 TXT 数据文件需要认证才能读取；敏感配置仍应保存在服务端数据目录，不要放入插件目录
- 插件管理 API 仅允许管理员调用，插件界面应对 403 响应给出明确提示
